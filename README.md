...
- `gd`：字面量 include 优先在清单内找头文件；重名时显示候选路径和预览。函数、结构体等
  符号先查询 LSP，失败或等待 2 秒后优先显示清单内的函数定义候选；未识别到定义时提示并显示全部同名文本。
- `gr`：先查询 LSP 语义引用；无结果时显示清单内同名文本，排除已识别的函数定义位置。
...
清单模式的 `gd/gr` 复用包内 Tree-sitter 做函数语法筛选，不需要 LSP 或编译数据库。
支持多行函数头、指针/自定义返回类型和单行函数体；函数原型不当作定义，递归调用仍保留在 `gr`。
